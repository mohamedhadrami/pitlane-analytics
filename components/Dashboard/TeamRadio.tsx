...
import type { OFDriver, OFSessionParams, OFTeamRadio } from "../../types/openF1.types";
...
const TeamRadios: React.FC<{ drivers: OFDriver[], teamRadio: OFTeamRadio[] }> = ({ drivers, teamRadio }) => {
    ...
    return (
        <div style={{ scrollbarWidth: "none" }} className="overflow-y-scroll">
            <ScrollShadow className="min-w-[300px] h-[50vh] m-1" size={50}>
                {sortedTeamRadio && gmtOffset && sortedTeamRadio.map((radio: OFTeamRadio, index: number) => (
                    <RadioItem key={`${radio.driver_number}-${radio.date}`} drivers={drivers} radio={radio} gmtOffset={gmtOffset} />
                ))}
            </ScrollShadow>
        </div>
    );
...
const RadioItem: React.FC<{ drivers: OFDriver[], radio: OFTeamRadio, gmtOffset: string }> = ({ drivers, radio, gmtOffset }) => {
